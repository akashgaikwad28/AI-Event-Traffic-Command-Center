...
class ModelTrainer:
    ...
    def __init__(self, cleaned_data_path: str, artifacts_dir: str):
        ...
        self.features = ['latitude', 'longitude', 'hour_of_day', 'day_of_week', 'is_weekend', 'is_peak_hour', 'priority_encoded', 'is_closed']
        
        # Initialize MLOps Components
        self.tracker = MLFlowTracker(experiment_name="Gridwise_AI_Traffic_Models")
        self.registry = RegistryManager(registry_file=str(self.artifacts_dir / "model_registry.json"))
    ...
    def train_and_export(self):
        ...
        self.tracker.start_run(run_name=f"Training_Run_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}")
        ...
        self.tracker.log_xgboost_model(congestion_model, "congestion_model", "Gridwise_Congestion_Proxy")
        self.tracker.log_sklearn_model(deployment_model, "deployment_model", "Gridwise_Deployment_Load")
        self.tracker.log_xgboost_model(response_model, "response_time_model", "Gridwise_Response_Time")
