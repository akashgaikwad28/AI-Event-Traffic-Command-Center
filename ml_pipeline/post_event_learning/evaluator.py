...
class PostEventLearningLoop:
    ...
    def evaluate_event_accuracy(
        self, event_id: str, predicted_gori: float, actual_gori: float
    ):
        """
        ...
        logger.info(
            f"Event {event_id}: Predicted GORI={predicted_gori}, Actual GORI={actual_gori}. Error={error}"
        )

        self.feedback_log.append(
            {"event_id": event_id, "error": error, "requires_retraining": error > 15.0}
        )

        self._trigger_retraining_if_needed()

    def evaluate_resource_effectiveness(
        self, deployment_id: str, officers_deployed: int, congestion_reduction: float
    ):
        """
        Evaluates the ROI of deployed manpower and barricades.
        """
        efficiency_score = (
            congestion_reduction / officers_deployed if officers_deployed > 0 else 0
        )
        logger.info(
            f"Deployment {deployment_id} achieved {congestion_reduction}% reduction using {officers_deployed} officers."
        )
    ...
    def _trigger_retraining_if_needed(self):
        ...
        """
        high_error_cases = [
            log for log in self.feedback_log if log["requires_retraining"]
        ]
        if len(high_error_cases) > 10:
            logger.warning(
                "Prediction drift detected! Triggering ml_pipeline/training job."
            )
