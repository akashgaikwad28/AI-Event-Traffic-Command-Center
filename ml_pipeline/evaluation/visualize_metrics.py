...
class ModelVisualizer:
    ...
    def plot_confusion_matrix(self, cm: list, class_names: list, filename: str = "deployment_confusion_matrix.png"):
        ...
        try:
            ...
            plt.title('Deployment Load Classification - Confusion Matrix', fontsize=14, pad=15)
    ...
    def plot_feature_importance(self, model, feature_names: list, title: str, filename: str):
        """Extracts and plots feature importances from a tree-based model."""
        try:
            if hasattr(model, 'feature_importances_'):
                importances = model.feature_importances_
            else:
                logger.warning(f"Model {type(model)} does not have feature_importances_")
                return
