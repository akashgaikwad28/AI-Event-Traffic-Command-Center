...
class GroqProvider(BaseProvider):
    ...
    async def generate_explanation(self, prompt: str, context: dict[str, Any]) -> str:
        ...
        print(
            f"DEBUG: API Key check -> Settings: {'[SET]' if settings.groq_api_key else '[MISSING]'}, Env: {'[SET]' if os.getenv('GROQ_API_KEY') else '[MISSING]'}"
        )
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                self.base_url, headers=headers, json=payload, timeout=8.0
            )
            ...
            data = response.json()
            print(
                "✅ GROQ API SUCCESS: Explanation generated successfully via llama3-8b-8192"
            )
