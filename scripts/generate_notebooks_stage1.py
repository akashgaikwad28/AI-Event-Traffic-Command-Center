...
set_seed(GLOBAL_RANDOM_SEED)"""),
        ...
        new_code_cell("""df = pd.read_csv(RAW_DATA_PATH)
print(f"Initial shape: {df.shape}")
df.head()"""),
        
        new_markdown_cell("## 4. Data Quality Report - Initial"),
        new_code_cell("""# Missing Values Summary
missing = df.isna().sum()
...
display(missing_df[missing_df['Missing Count'] > 0])"""),
        ...
        new_code_cell("""# Invalid Coordinate Report
invalid_coords = df[
...
"""),
        ...
        new_code_cell("""# Drop explicitly broken rows or handle them safely
...
print(f"Dropped {initial_len - len(df)} rows due to fundamental data corruption.")"""),
        ...
        new_code_cell("""# Fill essential missing values with 'unknown' or median
...
for col in fill_cols:
    if col in df.columns:
        ...
        new_code_cell("""# Create intermediate dir if not exists
...
"""),
        ...
        new_code_cell("""intermediate_path = PROCESSED_DATA_DIR / "intermediate_cleaned.parquet"
df = pd.read_parquet(intermediate_path)
print(f"Loaded {len(df)} rows.")"""),
        
        new_markdown_cell("## 4. Target Generation\nUsing operational proxies for realistic predictions."),
        new_code_cell("""# 1. Response Duration
...
display(df[[RESPONSE_TIME_TARGET, CONGESTION_TARGET, DEPLOYMENT_TARGET]].head())"""),
        ...
        new_markdown_cell("## 5. Feature Engineering\nExtracting temporal and spatial features."),
        new_code_cell("""# Time features from start_datetime
...
"""),
        ...
        new_markdown_cell("## 6. Leakage Validation\nEnsure no future information (e.g. resolved_datetime) leaks into predictive features."),
        new_code_cell("""# List of features meant for training (excluding targets and leakage columns)
