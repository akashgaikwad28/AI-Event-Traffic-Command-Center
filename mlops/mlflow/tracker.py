...
class MLFlowTracker:
    ...
    def __init__(self, experiment_name: str):
        ...
        dagshub.init(repo_owner='akash.gaikwad9945', repo_name='AI-Event-Traffic-Command-Center', mlflow=True)
        
        mlflow.set_tracking_uri("https://dagshub.com/akash.gaikwad9945/AI-Event-Traffic-Command-Center.mlflow")
        mlflow.set_experiment(experiment_name)
        
    def start_run(self, run_name: str):
        self.active_run = mlflow.start_run(run_name=run_name)
        logger.info(f"[MLFlow] Started run: {run_name} under experiment {self.experiment_name}")
    ...
    def log_xgboost_model(self, model, artifact_path: str, registered_model_name: str):
        mlflow.xgboost.log_model(model, artifact_path=artifact_path, registered_model_name=registered_model_name)
        logger.info(f"[MLFlow] Registered XGBoost Model - Name: {registered_model_name}")
    ...
    def log_sklearn_model(self, model, artifact_path: str, registered_model_name: str):
        ...
        logger.info(f"[MLFlow] Registered Sklearn Model - Name: {registered_model_name}")
